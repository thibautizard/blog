...
Plusieurs solutions sont possibles. Personellement, j'ai l'habitude de **créer des variables CSS** pour contrôler les propriétés qui m'intéressent et d'en modifier ensuite les valeurs dans les différents sous-sélecteurs de type `[data-status="XXX"]`.
...
.badge {
  ...
  color: if(
    style(--status: pending): #9333ea; style(--status: success): #16a34a;
      style(--status: error): #ef4444; else: #0ea5e9
  );

  background-color: if(
    style(--status: pending): #f5f3ff; style(--status: success): #dcfce7;
      style(--status: error): #fee2e2; else: #cffafe
  );
}
```

Tout se joue dans la définition d'une propriété CSS custom `--status` qui récupère automatiquement la valeur de l'attribut HTML `[data-status]` laquelle peut ensuite **être comparée pour assigner les styles adéquats si la condition est vérifiée**.
...
<div className="useful-links">
  #### Liens additionnels - 🔗 [Documentation
