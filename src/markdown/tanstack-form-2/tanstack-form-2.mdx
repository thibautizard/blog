...
Tout ce qui est retourné par cette fonction sera considéré comme une **erreur de validation** et automatiquement ajouté au tableau `field.state.meta.errors`. Si cette fonction ne retourne rien ou `null`, le champ est considéré **comme valide**.
...
```
 ...
 Jusqu'à présent, nous avons mis en place des validations simples qui se basent uniquement sur la valeur du champ déjà disponible côté client. Mais quid **si on souhaite vérifier la validité d'un champ en l'envoyant à une API** ? On passe alors à une validation asynchrone plus délicate car elle **nécessite d'attendre la réponse de l'API (qui peut être longue pu ne jamais arriver) pour déterminer si le champ est valide ou non**.

  C'est le moment de recourir à la propriété `onChangeAsync` qui sera voisine et complémentaire de `onChange` (mais il existe aussi `onBlurAsync` et `onSubmitAsync` qui sont les pendants de `onBlur` et `onSubmit`). Elle prendra en valeur **une fonction chargée d'effectuer les appels API nécessaires pour tester l'input**.
...
Ce point, ainsi que diverses améliorations que l'on peut apporter à notre formulaire grâce à TanStack Form, feront l'objet d'[une dernière partie](/post/tanstack-form-advanced).
