...
[Créer des formulaires modernes et type-safe avec TanStack Form - Partie 1](/post/tanstack-form-1)
...
Tout ce qui est retourné par cette fonction est considéré comme une **erreur de validation** et sera ajouté à `field.state.meta.errors`. Si cette fonction ne retourne rien ou `null`, le champ est considéré **comme valide**.
...
Si on ne souhaite éprouver la validité du formulaire qu'après que l'utilisateur lui-même estime avoir fini de le remplir, on en passera **par** `onSubmit` **défini au moment de la création du formulaire et qui sera exécuté à chaque appel de** `form.handleSubmit`.
...
Il est tout à fait possible de centraliser ses opérations de validation dans les `validators` du `form` plutôt qu'au niveau des `field` ce qui est **plus pratique si on souhaite re-valider plusieurs champs en même temps en fonction des interactions de l'utilisateur avec le formulaire**.
...
```
 ...
 Une fois **passées les validations locales côté client**, peuvent intervenir les validations côté serveur asynchrones par nature. C'est le moment de recourir à la propriété `onChangeAsync` qui sera voisine et complémentaire de `onChange` (mais il existe aussi `onBlurAsync` et `onSubmitAsync` qui sont les pendants de `onBlur` et `onSubmit`). Elle prendra en valeur une fonction chargée d'effectuer les appels API nécessaires pour tester l'input.
...
À noter que **la validation** `onChangeAsync` **ne se déclenchera qu'<u>après</u> les validations** `onChange` **réussies**. En suivant cet ordre, on évite de solliciter inutilement une API si l'input peut déjà être détecté comme invalide avant l'envoi d'une requête.
 
 #### Afficher une erreur de validation pour un champ

Pour vérifier que la validation spécifique à un champ fonctionne, on afficher le plus souvent sous notre `input` les messages d'erreur associés. Il suffit d'inspecter **la propriété** `field.state.meta.errors` **qui va nous livrer un tableau d'erreurs à exploiter du côté du rendu** (tableau vide si le champ est valide).
...
Cependant on risque de **ne pas afficher l'erreur** au moment où elle est créée ! Pourquoi ? Car par souci d'optimisation, **TanStack Form ne veut pas forcer React à rendre l'entièreté du composant contenant le formulaire à chaque modification du** `form`. 

Imaginons que le `<form>` soit pris dans un composant bien plus large : si le hook `useForm` qui retourne notre objet `form` est appelé à la hauteur de ce composant, une modification du formulaire pourrait entraîner le rendu de tous les éléments et composants enfants même s'ils n'ont rien à voir avec la partie dédiée au formulaire. Il en résulterait **une dégradation des performances si ces changements surviennent trop fréquemment**. 🤷‍♂️
...
Ce point ainsi que toutes les diverses améliorations que l'on peut apporter à notre formulaire grâce à TanStack Form feront l'objet d'[une dernière partie](/post/tanstack-form-advanced).
