...
Quitte à utiliser des `<button>` et des `<div>` pour contrefaire notre `<select>`, **pourquoi ne pas tenter une implémentation personnelle afin de ne pas se reposer sur une librairie externe pour un composant aussi essentiel ?**
...
Certes rien de fou à première vue, celui-ci paraît même moins esthétique ainsi dépouillé de ses plus beaux atours. Mais croyez-moi, ce `<select>` offre de toutes nouvelles possibilités.
...
Dorénavant, il est possible **d'insérer des éléments visuels (images, svg etc.)** qui étaient auparavant ignorés par le *parser* du navigateur. Exemple avec ce sélecteur de frameworks Javascript 👇
...
Plus globalement le `<select>` customisable accepte n'importe quel HTML dans ses `<option>` alors que le `<select>` classique récupérait uniquement le nœud textuel.

#### Customiser l'élément sélectionné

Souvent l'élément sélectionné a des contraintes d'affichage qui diffèrent de celles des options de la liste de choix possibles. La balise `<selectedcontent>` permet ainsi de cibler spécifiquement l'élément actuellement sélectionné et de lui appliquer ensuite des styles personnalisés.
...
Ce `<selectedcontent>` agit comme un *slot* à l'intérieur duquel se téléporte l'`<option>` actuellement sélectionnée.

Essayons ainsi de reproduire **le bouton Github pour choisir le type de merge** à l'issue d'une PR où l'option sélectionnée doit présenter un aspect différent de l'option affichée dans la liste👇
...
Lorsque vous passer en `appearance: base-select;` quelques différences subtiles sont à prendre en compte :

- Le `<select>` ne prend plus automatiquement la largeur de l'`<option>` la plus longue.
- Le `<select>` ne déclenche plus automatiquement les composants mobiles spécifiques.
