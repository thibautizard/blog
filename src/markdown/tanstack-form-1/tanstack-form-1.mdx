...
export const metadata = {
  title: "Créer des formulaires modernes et typesafe avec Tanstack Form 📝 [Partie 1 - Introduction]",
  date: "2025-07-28",
  excerpt:
...
Mais quelles qualités devrait présenter une **librairie de formulaires digne de ce nom** pour remplacer tout ce foutoir ?
...
Tout aussi indispensable,on peut y ajouter **une fonction** `onSubmit` qui sera appelée à chaque tentative de validation.
...
Première brique essentielle : **le composant** `form.Field` dont l'attribut `name` correspondra à l'un des champs du formulaire  déclarés dans `useForm()`.
...
Plutôt que de lui passer un `children`sous la forme `<form.Field>{children}</form.Field>`, on recourt ici à la bonne vieille technique des `renderProps` pour **fournir à notre input sous-jacent un objet `field` contenant les différents états du champ en temps réel** et adapter l'`input` qu'on souhaite rendre aux valeurs de celui-ci.

Dernière étape pour assurer la validation du formulaire : **désactiver le comportement par défaut du bouton de soumission qui survient lorsqu'un bouton** `<button type="submit">` se trouve dans votre élément `<form>`. Ceci est crucial pour empêcher le rechargement de la page qu'on souhaite éviter dans le cadre d'une SPA car celui-ci nous ferait sortir de notre routing et court-circuiterait les opérations de TanStack Form. On va plutôt lui préférer la méthode `form.handleSubmit()` pour déclencher les étapes de validations et soumettre notre formulaire.
...
Voilà en deux coups de cuillère à pot un bon début de formulaire de connexion. Ne reste plus qu'à **ajouter la validation des champs** et la **gestion des erreurs**.

<div className="useful-links">
  #### Liens additionnels - 🔗 
  - <span className="font-bold">📄</span> [Documentation officielle de TanStack Form](https://tanstack.com/form/latest/docs/overview)
  - <span className="font-bold">📺</span> [Tutos Youtube de Balastrong](https://www.youtube.com/watch?v=Pf1qn35bgjs&list=PLOQjd5dsGSxInTKUWTxyqSKwZCjDIUs0Y&index=1)
  - <span className="font-bold">📄</span> [LogRocket — TanStack Form vs. React Hook Form](https://blog.logrocket.com/tanstack-form-vs-react-hook-form)
</div>
